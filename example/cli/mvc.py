...
class BaseView:
    ...
    def __init__(self, window):
        ...
        # Create a sub-window for the top section
        ...
        # Create a sub-window for the middle section
        if self.middle_h > 1:
        ...
        # Create a sub-window for the bottom section
    ...
    def handle_resize(self):
        ...
        # Resize and clear the main window
        self.window.resize(self.max_y, self.max_x)
        self.window.clear()

        # Resize and/or clear the top sub-window
        if self.top_win:
            if self.max_y > self.top_h + self.bottom_h:
                self.top_h = len(self.LOGO)
        ...
        # Resize and/or clear the middle sub-window
        if self.middle_win:
            if self.middle_h > 1:
       ...
       # Resize and reposition the bottom sub-window
        ...
        if self.top_win:
            ...
            for y, line in enumerate(self.LOGO):
                if y < self.bottom_y:
                    ...
                    self.top_win.addnstr(y, x, line, self.max_x - 1, curses.A_BOLD)
        ...
        if self.middle_win:
            ...
            for y, line in enumerate(data["contents"]):
                if y < self.middle_h - 2:
                    self.middle_win.addnstr(y + 1, 2, line, self.max_x - 4) # adjust for window box edges
...
class Navigation:
    def __init__(self):
        self.current_view = 'topic'
        self.views = {
            'topic': TopicView,
            'consumergroup': ConsumerGroupView
        }

        self.models = {
            'topic': MOCK_TOPIC_DATA,
            'consumergroup': MOCK_CONSUMER_GROUP_DATA
        }
    ...
    def get_current_model(self):
        return self.models[self.current_view]
...
class Controller:
    ...
    def run(self):
        ...
        self.model.add_data(MOCK_TOPIC_DATA)

        while True:
            # data = self.model.data
            data = self.navigation.get_current_model()
